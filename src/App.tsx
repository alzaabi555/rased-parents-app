    ...
    <header className="sticky top-0 z-40 bg-white/80 backdrop-blur-2xl border-b border-indigo-100 pt-[max(env(safe-area-inset-top),16px)] pb-4 px-5 transition-all shadow-[0_4px_20px_rgba(0,35,102,0.05)]">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3 min-w-0">
          {showBack && (
            <button onClick={onBack} className="p-2.5 bg-indigo-50 hover:bg-indigo-100 shadow-sm border border-indigo-200 rounded-full text-[#002366] transition-all active:scale-95 shrink-0">
              <ArrowLeft size={20}/>
            </button>
          )}
          {icon && !showBack && (
      ...
      >
        <div className="space-y-5">
          <section className="grid grid-cols-2 gap-3">
            <div className="bg-white p-4 rounded-2xl shadow-sm border border-indigo-50 flex items-center gap-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <section className="bg-white rounded-3xl p-5 border border-indigo-50 shadow-sm flex flex-col max-h-[300px]">
              ...
              <div className="space-y-1.5 overflow-y-auto custom-scrollbar pr-1 flex-1">
                {displayPos.length > 0 ? displayPos.map((b: any, i: number) => (
                  <div key={i} className="flex justify-between items-center p-3 rounded-2xl bg-emerald-50/30 border border-emerald-100 transition-all hover:bg-emerald-50">
            ...
            <section className="bg-white rounded-3xl p-5 border border-indigo-50 shadow-sm flex flex-col max-h-[300px]">
              ...
              <div className="space-y-1.5 overflow-y-auto custom-scrollbar pr-1 flex-1">
                {neg.length > 0 ? neg.map((b: any, i: number) => (
                  <div key={i} className="flex justify-between items-center p-3 rounded-2xl bg-rose-50/30 border border-rose-100 transition-all hover:bg-rose-50">
                    <div className="flex items-center gap-2.5">
          ...
          <section className="bg-white rounded-3xl p-5 border border-indigo-50 shadow-sm flex flex-col max-h-[350px]">
            <h3 className="text-sm font-black text-[#002366] mb-3 flex items-center gap-2 px-1 shrink-0">
                <ClipboardList size={18} className="text-[#002366]"/> سجل الدرجات
            </h3>
            <div className="rounded-2xl border border-indigo-50 overflow-hidden overflow-y-auto custom-scrollbar flex-1 bg-indigo-50/10">
                ...
                <tbody className="divide-y divide-indigo-50">
                  {s.grades?.map((g: any, i: number) => (
                    <tr key={i} className="hover:bg-white transition-colors">
                      ...
                      <td className="py-3.5 px-4 text-center"><span className="text-sm font-black text-[#002366] bg-indigo-50 px-3 py-1 rounded-lg border border-indigo-100">{g.score}</span></td>
                    </tr>
                  ))}
...
function App() {
  ...
  const renderDashboard = () => (
    <div className="animate-in fade-in duration-500 h-full">
      <GlassLayout
        ...
        icon={<User size={24} />}
        rightAction={
          <button onClick={() => fetchStudentData(civilID, true)} disabled={isRefreshing} className="p-2.5 rounded-xl bg-indigo-50 border border-indigo-100 text-[#002366] active:scale-95 shadow-sm">
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {allSubjects.map((sub, idx) => (
            <div key={idx} onClick={() => setSelectedSubject(sub)} className="bg-white rounded-[2rem] p-5 shadow-sm border border-indigo-50 hover:border-indigo-300 hover:shadow-md cursor-pointer active:scale-[0.98] transition-all">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-2xl bg-indigo-50 flex items-center justify-center text-[#002366] border border-indigo-100"><BookOpen size={20} /></div>
                  <div>
              ...
              <div className="mt-5 flex items-center gap-3">
                <div className="flex-1 h-1.5 bg-indigo-50 rounded-full overflow-hidden border border-indigo-50">
    ...
    return (
      <div className="animate-in fade-in duration-500 h-full">
        <GlassLayout title="الإشعارات" icon={<Bell size={24} />}>
          <div className="flex p-1 bg-white border border-indigo-50 rounded-xl mb-6 shadow-sm">
          ...
          <div className="space-y-3">
            {alerts.filter(a => activeAlertTab === 'all' || a.kind === 'alert').map((item, idx) => (
              <div key={idx} className="bg-white rounded-2xl p-4 shadow-sm border border-indigo-50 transition-all hover:-translate-y-1">
  ...
  const renderProfile = () => (
    <div className="animate-in fade-in duration-500 h-full">
      <GlassLayout title="حسابي" icon={<User size={24} />}>
          <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-indigo-50 text-center mb-6">
              ...
              <p className="text-[#002366] text-sm font-bold bg-indigo-50 py-2 px-6 rounded-xl border border-indigo-100 inline-block">{allSubjects[0]?.name}</p>
          </div>
          <button onClick={handleLogout} className="w-full bg-rose-50 hover:bg-rose-100 border border-rose-100 text-rose-600 py-5 rounded-2xl font-black flex items-center justify-center gap-3 transition-all active:scale-95 shadow-sm"><LogOut size={20} /> تسجيل الخروج</button>
  ...
  return (
    <div className="h-[100dvh] w-full relative overflow-hidden bg-[#f0f4f8] font-sans text-slate-800" dir="rtl">
      ...
      <nav className="absolute bottom-0 left-0 right-0 z-[90] flex justify-around items-center px-4 pb-[calc(env(safe-area-inset-bottom)+0.5rem)] pt-3 bg-white/90 backdrop-blur-2xl shadow-[0_-10px_40px_rgba(0,35,102,0.08)] border-t border-indigo-50 transition-all">
        <button onClick={() => { setCurrentTab('home'); setSelectedSubject(null); }} className={`flex flex-col items-center justify-center px-4 py-2 transition-all duration-300 ${currentTab === 'home' ? 'text-[#002366] scale-110 -translate-y-1' : 'text-slate-400 hover:text-indigo-400'}`}>
          <LayoutGrid size={22} className={currentTab === 'home' ? 'fill-indigo-50' : ''} /><span className="text-[9px] font-black mt-1">الرئيسية</span>
        </button>
        <button onClick={() => { setCurrentTab('alerts'); setSelectedSubject(null); }} className={`flex flex-col items-center justify-center px-4 py-2 transition-all duration-300 relative ${currentTab === 'alerts' ? 'text-[#002366] scale-110 -translate-y-1' : 'text-slate-400 hover:text-indigo-400'}`}>
          <Bell size={22} className={currentTab === 'alerts' ? 'fill-indigo-50' : ''} /><span className="absolute top-1 right-3 w-2 h-2 bg-rose-500 rounded-full border border-white"></span><span className="text-[9px] font-black mt-1">إشعارات</span>
        </button>
        <button onClick={() => { setCurrentTab('profile'); setSelectedSubject(null); }} className={`flex flex-col items-center justify-center px-4 py-2 transition-all duration-300 ${currentTab === 'profile' ? 'text-[#002366] scale-110 -translate-y-1' : 'text-slate-400 hover:text-indigo-400'}`}>
          <User size={22} className={currentTab === 'profile' ? 'fill-indigo-50' : ''} /><span className="text-[9px] font-black mt-1">حسابي</span>
        </button>
      </nav>
      {isMessageOpen && (
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-end sm:items-center justify-center sm:p-4">
          <div className="bg-white w-full sm:max-w-sm rounded-t-[2.5rem] sm:rounded-[2.5rem] p-7 shadow-2xl animate-in slide-in-from-bottom-8 sm:zoom-in-95">
