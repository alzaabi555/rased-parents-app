...
function App() {
  ...
  if (showWelcomeScreen && allSubjects.length > 0) {
    return (
      <div className={`h-screen flex flex-col items-center justify-center p-6 font-sans text-center relative overflow-hidden animate-in fade-in duration-500 ${isRamadan ? 'bg-gradient-to-br from-[#0f172a] to-[#1e1b4b]' : 'bg-gradient-to-br from-[#1e3a8a] to-blue-700'}`} dir="rtl">
        ...
        <div className="absolute top-[-10%] left-[-10%] w-64 h-64 bg-amber-400/20 rounded-full blur-3xl z-0"></div>
        <div className="absolute bottom-[-10%] right-[-10%] w-64 h-64 bg-indigo-500/20 rounded-full blur-3xl z-0"></div>
        
        {/* أضفنا relative z-20 لرفع المحتوى */}
        <div className="relative z-20 flex flex-col items-center animate-in slide-in-from-bottom-8 duration-700">
  ...
  if (allSubjects.length > 0 && !selectedSubject) {
    return (
      <div className={`h-screen flex flex-col font-sans overflow-hidden animate-in fade-in duration-500 ${isRamadan ? 'bg-[#0f172a]' : 'bg-slate-50'}`} dir="rtl">
        ...
        {/* Header - أضفنا relative z-20 */}
        <div className={`px-6 pt-12 pb-8 rounded-b-[2.5rem] shadow-xl shrink-0 relative z-20 overflow-hidden ${isRamadan ? 'bg-[#1e1b4b] border-b border-indigo-500/20' : 'bg-gradient-to-br from-[#1e3a8a] to-blue-600'}`}>
        ...
        {/* Subjects List - أضفنا relative z-20 */}
        <div className="flex-1 overflow-y-auto px-6 mt-4 pb-20 space-y-4 custom-scrollbar relative z-20">
  ...
  if (selectedSubject) {
    ...
    return (
      <div className={`h-screen flex flex-col font-sans relative overflow-hidden ${isRamadan ? 'bg-[#0f172a] text-white' : 'bg-slate-50 text-slate-800'}`} dir="rtl">
        ...
        {/* Message Modal - رفعناه للطبقة 100 */}
        {isMessageOpen && (
        ...
        {/* Dashboard Header - أضفنا relative z-20 */}
        <div className={`px-6 pt-12 pb-8 rounded-b-[2.5rem] shadow-lg shrink-0 relative z-20 ${isRamadan ? 'bg-[#1e1b4b] border-b border-indigo-500/20' : 'bg-[#1e3a8a] text-white'}`}>
          <div className="flex justify-between items-center mb-4">
        ...
        {/* Dashboard Content - أضفنا relative z-20 */}
        <div className="px-5 -mt-4 space-y-5 flex-1 overflow-y-auto pt-8 pb-32 custom-scrollbar relative z-20">
  ...
  return (
    <div className={`min-h-screen flex flex-col justify-between items-center p-6 font-sans overflow-hidden relative ${isRamadan ? 'bg-gradient-to-br from-[#0f172a] to-[#1e1b4b]' : 'bg-gradient-to-br from-[#0f172a] to-[#1e3a8a]'}`} dir="rtl">
      ...
      <div className={`absolute top-[-10%] left-[-10%] w-64 h-64 rounded-full mix-blend-screen filter blur-3xl z-0 ${isRamadan ? 'bg-indigo-500/10' : 'bg-blue-500/20'}`}></div>
      <div className={`absolute bottom-[-10%] right-[-10%] w-64 h-64 rounded-full mix-blend-screen filter blur-3xl z-0 ${isRamadan ? 'bg-amber-500/10' : 'bg-amber-500/20'}`}></div>
      ...
      {/* أضفنا relative z-20 لرفع الفوتر */}
